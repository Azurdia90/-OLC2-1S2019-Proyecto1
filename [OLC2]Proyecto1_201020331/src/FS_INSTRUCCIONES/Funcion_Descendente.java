 ...
 */
public class Funcion_Descendente implements Instruccion
{
    private int fila;
    private int columna;
    ...
    public Funcion_Descendente(Nodo_AST_FS nodo_funcion, FS_Arreglo p_arreglo)
    {
    ...
    public Simbolo ejecutar(Entorno entorno_local, ObjetoEntrada salida) 
    {
        try
        {
            ...
            {
                Collections.sort(arreglo, (Simbolo p1, Simbolo p2) -> new Integer(p1.getValor().toString()).compareTo(new Integer(p2.getValor().toString())));
            }
            else if(arreglo.getHomogeneo() && arreglo.getTipo() == Tabla_Enums.tipo_primitivo_Simbolo.decimal)
            {
                Collections.sort(arreglo, (Simbolo p1, Simbolo p2) -> new Double(p1.getValor().toString()).compareTo(new Double(p2.getValor().toString())));
            }
            else if(arreglo.getHomogeneo() && arreglo.getTipo() == Tabla_Enums.tipo_primitivo_Simbolo.cadena)
            {
                Collections.sort(arreglo, (Simbolo p1, Simbolo p2) -> p1.getValor().toString().compareTo(p2.getValor().toString()));
            }
            else
            {
            ...
            nuevo_simbolo.setValor("La llamada a funciones de un Arreglo fue realizada");
